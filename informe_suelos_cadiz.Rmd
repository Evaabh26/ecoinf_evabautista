...
Estas se encuentranen la provincia de Cádiz, próximas a espacios protegidos como el *Parque Natural Bahía de Cádiz* y varias *Reservas Naturales Endorreicas*. 

Los usos del suelo predominantes en los emplazamientos de Puerto Real 110, Cartuja y 
Las Quinientas son los Cultivos agrícolas (Cultivo herbáceo distinto de arroz), mientras que Mira-mundo se asienta sobre una Vía de comunicación no asfaltada. Las cuatro parcelas presentan la existencia de un Hábitat de Interés Comunitario (HIC) terrestre no prioritario.
...
Los valores de los indicadores edáficos y topográficos se obtienen a partir de cartografía y fuentes de información ambiental (como REDIAM o Catastro), mientras que los climáticos se pueden estimar a partir de datos meteorológicos de una estación del IFAPA. 
...
$$
\begin{equation}
Riesgo de heladas (nºmeses) = 11,441 –(0,758 \times T)\end{equation}
$$
donde:
...
En la Figura @ref(fig:grafico-pp) se observa la variabilidad de las precipitaciones diarias durante el periodo de estudio.
...
En la Figura @ref(fig:grafico-pp) se observa la variabilidad de la temperatura diaria durante el periodo de estudio.
...
La determinación de la clase final de la parcela se realiza aplicando la Ley del Mínimo, una práctica esencial en este tipo de evaluación. Esto significa que la 
clasificación final estará determinada por el parámetro que presente la limitación más restrictiva (el peor valor)
...
Por el contrario, las centrales de *Cartuja* y *Las Quinientas* se sitúan en suelos de **Clase S2 (Buena a Moderada)** dedicados actualmente a cultivos herbáceos activos, lo que genera un conflicto de uso del suelo al desplazar producción agrícola por infraestructuras industriales. En el caso de *Miramundo*, aunque posee una calidad intrínseca elevada **(S1-S2)**, el impacto directo sobre la pérdida de producción primaria se ve minimizado al asentarse sobre una vía de comunicación no asfaltada [@lopez_geta_atlas_2005]
